...
export const TmuxAgentStatusPlugin: Plugin = async ({ $, client }) => {
	...
	const isSubagentSession = async (sessionID: string): Promise<boolean> => {
		...
		try {
			...
			await appendLog(
				`[${timestamp()}] ERROR scope=session.check sessionID=${sessionID} caught=${JSON.stringify(describeError(error))}`,
			)
			return false
		}
	}
	...
	const renameWindow = async (icon: string): Promise<void> => {
		try {
			...
			await appendLog(
				`[${timestamp()}] ERROR scope=renameWindow caught=${JSON.stringify(describeError(error))}`,
			)
		}
	}
